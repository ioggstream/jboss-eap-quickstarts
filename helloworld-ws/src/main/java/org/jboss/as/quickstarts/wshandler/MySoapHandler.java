 ...
 *  $ ./pre-handler-chain=my-handlers/handler=addtag-handler:add(class="org.jboss.as.quickstarts.wshandler.MySoapHandler")
 *  $ :reload
 * 
...
public class MySoapHandler implements SOAPHandler<SOAPMessageContext>  {
    ...
    */
    ...
    public boolean handleMessage(SOAPMessageContext mCtx) {
        ...
        try {
            ...
            if (outbound) {                
                ...
                // modify body using javax.xml.soap
                // as the request takes only one parameter, take it...
                SOAPBodyElement soapBodyElement = (SOAPBodyElement)body.getChildElements().next();
                //.. and get the first (or unique) argument of the parameter...
                SOAPElement soapElement = (SOAPElement)soapBodyElement.getChildElements().next();
                //...finally get its value...
                String value = soapElement.getValue();
                //..and replace the old one appending a known string
                soapElement.setValue(value + TOKEN);
            } else { 
    ...
    public Set<QName> getHeaders() {
        log.warning(String.format("MySoapHandler.getHeader() CDI context: %s", context));		
        return null;
    }
